...
def test_audit_and_notify_signal_deduplicates():
    with patch("app.telegram_notify.send_message") as mock_send, \
         patch("app.ai_copilot.analyze_trade_setup") as mock_analyze:
...
def test_audit_and_notify_signal_respects_config_toggle():
    with patch("app.telegram_notify.send_message") as mock_send, \
         patch.object(config, "ENABLE_AI_TELEGRAM_ALERTS", False):

...
function ScoreBar({ score }) {
  ...
  const color = pct >= 70 ? "bg-bull" : pct >= 40 ? "bg-accent-amber" : "bg-bear";
  return (
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold text-primary">Smart Money — Pre-Breakout Ranking</h1>
          </div>
          {data?.computed_at && (
            <div className="text-[11px] text-faint flex items-center gap-1.5">
              <RefreshCw size={11} />
              Updated {new Date(data.computed_at).toLocaleTimeString("en-IN", { hour12: false })}
            </div>
          )}
        </div>
        ...
        <div className="mb-4 flex items-start gap-2 rounded-xl border border-accent-blue/30 bg-accent-blue/10 px-4 py-3 text-xs text-primary">
          ...
          <div>
            Ranks the F&amp;O universe every 5 minutes on Fresh Turnover Ratio (50%), RVOL (30%)
            and Relative Strength vs NIFTY (20%) — each stock's turnover/volume compared against
            its own last {lookback} trading days in the same 5-min slot.{" "}
            <b>This is a watchlist filter, not a trade signal</b> — enter only when your own
            technical trigger (ORB, PDH, etc.) fires on a stock that shows up here.
            {buildingHistory && (
              <div className="mt-1.5 text-accent-amber font-semibold">
                Building history: candle_history only started recording on 2026-08-06, so most/all
                stocks are still below the {minDays}-day minimum needed for a reliable ratio.
                Rankings will start appearing as more trading days accumulate (fully warmed up
                after {lookback} trading days).
              </div>
            )}
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {(data?.top || []).map((row, i) => (
                  ...
                  >
                    <td className="py-3 px-4 text-faint font-mono text-xs">{i + 1}</td>
